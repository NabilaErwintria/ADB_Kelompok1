...
# === 2. Load Dataset ===
df = pd.read_csv(r"E:\\Dokument\Semester 7\\Analisis Data Bisnis\\Project_UAS\\lendingclub_feature engineer_tanpa_outlier.csv")
print(df.columns)
...
# === 7. Handling Imbalance with SMOTE (Only on Training Data) ===
...
for name, model in models.items():
    ...
    # Save model
    model_filename = f"{name.lower().replace(' ', '_')}_model_tanpa_outlier.pkl"
...
plt.ylabel("Score")
plt.title("Model Performance Comparison (SMOTE Applied)")
plt.ylim(0, 1)
plt.legend()
plt.show()

# === 12. Save Training Results to CSV ===
results_df.to_csv("model_results_with_smote_tanpa_outlier.csv", index=False)
print("\n📁 Hasil evaluasi telah disimpan ke: model_results_with_smote_tanpa_outlier.csv")
